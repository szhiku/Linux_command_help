...
- 创建目录：`mkdir Dir1`
	...
	- 语法格式 : mkdir [参数] 目录名
		...
		- 递归创建一系列目录和同级多个目录：`mkdir mkdir -pv /home/{ dirA/{ dir1,dir2} , dirB}`
	- 常用参数：
	- -m 创建目录的同时设置权限
...
- 复制：`cp anaconda-ks.cfg kickstart.cfg`复制单个文件，`cp anaconda-ks.cfg ks.cfg /etc`将多个文件一同复制到/etc目录中，如已有目标文件名称则默认询问是否覆盖
	- `cp -rfn`递归地复制目录和其内容，同时在复制过程中不会覆盖已存在的目标文件
...
- 删除：`rm anaconda-ks.cfg`
	...
	- ```
	  rm -rf Documents
	  ```
	- 强制删除当前工作目录内的所有以.txt为后缀的文件（*不包含隐藏文件）
	- ```
	  rm -f *.txt
	  ```
	- 常用参数：
